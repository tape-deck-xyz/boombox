...
Deno.test("ensureInfoJsonSeededAtStartup does not overwrite S3 when HEAD throws", async () => {
  setupStorageEnv();
  mockFilesWithAlbum();
  resetMockInfoJsonObject();
  const { regenerateInfoCache } = await import("../../server/info.ts");
  await regenerateInfoCache(new Request("http://seed-head-fail.example/"));
  ...
  setSendBehavior((command: unknown) => {
    ...
    if (key === "info.json" && name === "HeadObjectCommand") {
      return Promise.reject(new Error("mock transient HEAD failure"));
    }
