...
Deno.test("ensureInfoJsonSeededAtStartup does not overwrite S3 info.json when HEAD fails", async () => {
  setupStorageEnv();
  mockFilesWithAlbum();
  clearSendCalls();

  const { regenerateInfoCache } = await import("../../server/info.ts");
  await regenerateInfoCache(new Request("http://head-failure.example/"));
  ...
  setSendBehavior((command: unknown) => {
    ...
    if (name === "HeadObjectCommand" && key === "info.json") {
      return Promise.reject(new Error("transient HEAD failure"));
    }
