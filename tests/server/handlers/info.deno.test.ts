...
Deno.test("Info handler returns 503 when refresh cannot publish info.json to S3", async () => {
  setupStorageEnv();
  setupAdminEnv();
  mockFilesWithAlbum();
  ...
  try {
    ...
    assertEquals(await response.text(), "Could not publish info.json to S3");
  } finally {
