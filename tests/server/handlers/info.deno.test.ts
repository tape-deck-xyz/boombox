...
Deno.test("Info handler does not return 304 with an old ETag after local catalog changes", async () => {
  setupStorageEnv();
  setupAdminEnv();
  mockFilesWithAlbum();

  const first = await handleInfo(
    new Request("http://etag-change.example/info?refresh=1", {
      method: "GET",
      headers: { Authorization: createAdminAuthHeader() },
    }),
    {},
  ...
  setSendBehavior((command: unknown) => {
    ...
    if (name === "ListObjectsV2Command") {
      return Promise.resolve({
        Contents: [
          {
            Key: "Fresh%20Artist/Fresh%20Album/1__Fresh%20Track.mp3",
  ...
  try {
    ...
    const second = await handleInfo(
      new Request("http://etag-change.example/info", {
        method: "GET",
    ...
    assertEquals(second.headers.get("ETag") === oldEtag, false);
    const body = await second.json();
    assertEquals(Object.keys(body.contents), ["Fresh Artist"]);
  } finally {
