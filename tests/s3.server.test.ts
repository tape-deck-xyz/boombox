...
Deno.test("getUploadedFiles retries after a transient S3 listing failure", async () => {
  setupEnv();
  clearS3SendCalls();
  const now = new Date();
  let listCalls = 0;
  setSendBehavior((command) => {
    ...
    if (name === "ListObjectsV2Command") {
      ...
      return Promise.resolve({
        Contents: [
          {
            Key: "Recovered Artist/Recovered Album/1__Recovered.mp3",
            LastModified: now,
          },
        ],
        IsTruncated: false,
  ...
  assertEquals(files["Recovered Artist"]["Recovered Album"].tracks.length, 1);
});
