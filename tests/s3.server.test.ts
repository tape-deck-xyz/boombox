...
Deno.test("getUploadedFiles retries after a transient S3 ListObjectsV2 rejection", async () => {
  setupEnv();
  clearS3SendCalls();
  setSendBehavior((command) => {
    const name = (command as { constructor: { name: string } }).constructor
  ...
  setSendBehavior((command) => {
    ...
    if (name === "ListObjectsV2Command") {
      return Promise.resolve({
        Contents: [{ Key: "Recovered/Album/1__Track.mp3", LastModified: now }],
        IsTruncated: false,
      });
    }
    return Promise.resolve({});
  ...
  assertEquals(files["Recovered"]["Album"].tracks.length, 1);
});
