...
Deno.test("getUploadedFiles retries after a failed fetch instead of caching the rejection", async () => {
  setupEnv();
  clearS3SendCalls();
  let listAttempts = 0;
  setSendBehavior((command) => {
    ...
    if (name === "ListObjectsV2Command") {
      ...
      return Promise.resolve({
        Contents: [
          {
            Key:
              "Recovered%20Artist/Recovered%20Album/1__Recovered%20Track.mp3",
  ...
  try {
    ...
    assertEquals(Object.keys(files["Recovered Artist"]), ["Recovered Album"]);
  } finally {
    setSendBehavior(defaultSendBehavior);
