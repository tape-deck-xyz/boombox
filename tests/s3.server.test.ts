...
Deno.test("getUploadedFiles retries after a failed fetch instead of caching rejection", async () => {
  setupEnv();
  clearS3SendCalls();
  const now = new Date();
  let listAttempts = 0;
  setSendBehavior((command) => {
    ...
    if (name === "ListObjectsV2Command") {
      ...
      return Promise.resolve({
        Contents: [{ Key: "Artist/Album/1__Track.mp3", LastModified: now }],
        IsTruncated: false,
      });
    }
    return Promise.resolve({});
  ...
  assertEquals(files["Artist"]["Album"].tracks[0].title, "Track.mp3");
});
