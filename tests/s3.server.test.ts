...
Deno.test("getUploadedFiles retries after a failed S3 listing", async () => {
  ...
  setSendBehavior((command) => {
    const name = (command as { constructor: { name: string } }).constructor
      ?.name;
    if (name === "ListObjectsV2Command") {
      return Promise.reject(new Error("temporary S3 outage"));
    }
    return Promise.resolve({});
  });
  ...
  setSendBehavior((command) => {
    ...
    if (name === "ListObjectsV2Command") {
      return Promise.resolve({
        Contents: [
          {
            Key: "Recovered Artist/Recovered Album/1__Recovered Track.mp3",
  ...
  assertEquals(Object.keys(files), ["Recovered Artist"]);
  assertEquals(
    files["Recovered Artist"]["Recovered Album"].tracks[0].title,
    "Recovered Track.mp3",
  );
});
