...
async function waitForSelectedFileMetadata(dialog: Element): Promise<void> {
  const fileItem = dialog.querySelector("upload-dialog-file-item") as
    | ({ metadataReady: Promise<void> } & Element)
    | null;
  assertExists(fileItem);
