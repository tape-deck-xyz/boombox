...
export async function handleUpload(req: Request): Promise<Response> {
  ...
  try {
    ...
    // Force refresh and regenerate the shared catalog only after successful uploads.
    // The regeneration path serializes listing + writes so overlapping uploads do
    // not let an older S3 listing overwrite a newer info.json.
    if (successCount > 0) {
      try {
